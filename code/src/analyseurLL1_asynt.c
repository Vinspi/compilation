...
n_exp* expressionBis(n_exp *herite){
  ...
  if(CC == OU){
    ...
    CC = yylex();
    conj = conjonction();
    exp = expressionBis(cree_n_exp_op(ou,herite,conj));
    affiche_balise_fermante(__FUNCTION__,trace_xml);
    return exp;
...
n_exp* conjonctionBis(n_exp *herite){
  ...
  if(CC == ET){
    ...
    CC = yylex();
    comp = comparaison();
    conjbis = conjonctionBis(cree_n_exp_op(et,herite,comp));
    affiche_balise_fermante(__FUNCTION__,trace_xml);
    return conjbis;
  }
...
n_exp* expArithBis(n_exp *herite){
  ...
  if(CC == PLUS){
    ...
    CC = yylex();
    term = terme();
    exparithbis = expArithBis(cree_n_exp_op(plus,herite,term));
    affiche_balise_fermante(__FUNCTION__,trace_xml);
    return exparithbis;
  }
  if(CC == MOINS){
    ...
    CC = yylex();
    term = terme();
    exparithbis = expArithBis(cree_n_exp_op(moins,herite,term));
    affiche_balise_fermante(__FUNCTION__,trace_xml);
    return exparithbis;
  }
...
n_exp* termeBis(n_exp *herite){
  ...
  if(CC == FOIS){
    ...
    CC = yylex();
    neg = negation();
    termebis = termeBis(cree_n_exp_op(fois,herite,neg));
    affiche_balise_fermante(__FUNCTION__,trace_xml);
    return termebis;
  }
  if(CC == DIVISE){
    ...
    CC = yylex();
    neg = negation();
    termebis = termeBis(cree_n_exp_op(divise,herite,neg));
    affiche_balise_fermante(__FUNCTION__,trace_xml);
    return termebis;
  }
